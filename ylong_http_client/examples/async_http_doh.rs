...
use ylong_http_client::async_impl::{Body, Client, DohResolver, Downloader, Request};
...
async fn connect() -> Result<(), HttpClientError> {
    ...
    let doh_resolver = DohResolver::new("https://1.12.12.12/dns-query")
        .add_doh_server("https://120.53.53.53/dns-query");
    ...
    let _ = Downloader::console(doh_response).download().await;
